...
export const TopicGeneratorPage = () => {
  ...
  const generateWithGemini = async (formData: TopicGenerationValues) => {
    console.log('Calling backend for topic generation:', formData);
    ...
    try {
      ...
      if (data.success && data.topics && data.topics.length > 0) {
        // Return the first topic for now (backend returns multiple)
        const firstTopic = data.topics[0];
        console.log('Using first topic:', firstTopic);
        return firstTopic.title || firstTopic;
      } else {
        console.error('No topics in backend response:', data);
        throw new Error('No topics generated by backend');
      }
